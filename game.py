...
- 地图上存在多种道具，获得道具的agent能够获得相应的能力，有助于取得优势，道具包括：
    - 隐身道具，获得隐身道具的agent可以获得隐身能力，无法被对手看到，持续N回合，只有防守方可以获得隐身道具
    - 穿墙道具，获得穿墙道具的agent可以获得无视障碍物移动的能力，持续N回合
    - 金币，吃到金币可以获得分数奖励，只有防守方可以获得金币
    - 视野增加道具，获得道具的agent视野范围会扩大N格,持续M回合
    - 防守道具，获得防守道具的agent可以获得免疫攻击的能力，持续N回合，只有防守方可以获得防守道具
    - 所有道具会在地图的固定区域内生成，但是每个网格上具体生成哪些道具由随机数控制
- 除了道具之外，地图中还存在其他元素，包括：
    - 墙体：agent无法移动到墙体上面，也无法穿越墙体
    - 传送门：地图上存在成对出现的传送门，从传送门一端进入会立即从另一端出来
- 碰撞规则，以下情形会发生碰撞：
    - 攻击方或者防守方内部的agent之间如果出现目的地和路径冲突时会发生碰撞；如果出现了目的地冲突，序号小的agent会成功移动到目的地，序号大的会留在原地；如果出现路径冲突，则所有的agent会留在原地
    - 所有agent和墙体会发生碰撞，但是持有穿墙道具的agent除外
    - agent无法移动出地图边界，如果尝试会留在原地
    - 当攻击方的agent和防守方的agent发生碰撞的时候认为攻击方成功捕捉到防守方，防守方会立即回到出生地，攻击方会移动到目的地
